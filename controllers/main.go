...
import (
	...
	"github.com/Decred-Next/dcrnstakepool/poolapi"
...
func NewMainController(cfg *Config) (*MainController, error) {
	...
	mc.DCRDataURL = cfg.DCRNData //"http://13.56.184.70:17778"
	if mc.DCRDataURL == "" {
		mc.DCRDataURL = "https://testdata.dcrn.xyz"
	}
	return mc, nil
}
...
func (controller *MainController) API(c web.C, r *http.Request) *system.APIResponse {
	...
	var err error
	log.Infof("method:%s c:%s", r.Method, command)
	switch r.Method {
	case "GET":
		switch command {
...
func (controller *MainController) APIAddress(c web.C, r *http.Request) ([]string, codes.Code, string, error) {
	...
	log.Infof("userFee addr:%s script:%s", userFeeAddr.Address(), hex.EncodeToString(userFeeAddr.ScriptAddress()))
...
func (controller *MainController) AddressPost(c web.C, r *http.Request) (string, int) {
	...
	log.Infof("web userFee:%s addr:%s", userFeeAddr.String(), userFeeAddr.Address())
