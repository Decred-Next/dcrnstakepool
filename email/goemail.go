...
func (s *SMTP) Send(msg *Message) error {
	...
	switch s.scheme {
	case "smtps":
		conn, err = tls.Dial("tcp", serverAddr, s.tlsConfig)
	case "tls":
		fallthrough
	default:
